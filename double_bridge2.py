...
def rect_dot(frame, area1, area2, number): 
    ...
    if rect1[0][0] > rect2[0][0]: # 判断两个矩形位置，根据位置确定取的直线, rect1是右边的，rect2是左边的
        if len(approxCourve1) == 4 & len(approxCourve2) == 4:
            if number == 1:
                return length_line(approxCourve1[0], approxCourve1[3]) + length_line(approxCourve2[0], approxCourve2[1])
            elif number == 2:
                return nei_abf(approxCourve1[0], approxCourve1[1]), nei_abf(approxCourve2[0], approxCourve2[3])
    else:
        if len(approxCourve1) == 4 & len(approxCourve2) == 4:
            if number == 1:
                return length_line(approxCourve1[0], approxCourve1[1]) + length_line(approxCourve2[0], approxCourve2[3])
            elif number == 2:
...
while cap.isOpened():
    ...
    try:
        ...
        print(nei_median)
        if (nei_median<0.1) & (nei_median>-0.1): #TODO 判断直行的条件
            print("无需转向")
