...
hsv_low = np.array([17,40,131])
hsv_high = np.array([3,100,255])
...
def rect_dot(frame, area1, area2, number): 
    ...
    approxCourve1= cv2.approxPolyDP(area1,10,closed = True)
    approxCourve1 = np.array(approxCourve1)
    approxCourve1 = np.squeeze(approxCourve1, axis=1)
    approxCourve2= cv2.approxPolyDP(area2,10,closed = True)
    ...
    if rect1[0][0] > rect2[0][0]: # 判断两个矩形位置，根据位置确定取的直线, rect1是右边的，rect2是左边的
        if len(approxCourve1) == 4 and len(approxCourve2) == 4:
            if number == 1:
                return length_line(approxCourve1[2], approxCourve1[3]) + length_line(approxCourve2[2], approxCourve2[1])
            elif number == 2:
                return nei_abf(approxCourve1[0], approxCourve1[1]), nei_abf(approxCourve2[0], approxCourve2[3])
        else:
            if number == 1:
                return 0
            elif number == 2:
        ...
        if len(approxCourve1) == 4 and len(approxCourve2) == 4:
            if number == 1:
                return length_line(approxCourve1[2], approxCourve1[1]) + length_line(approxCourve2[2], approxCourve2[3])
            elif number == 2:
                return nei_abf(approxCourve1[0], approxCourve1[3]), nei_abf(approxCourve2[0], approxCourve2[1])
        else:
            if number == 1:
                return 0
            elif number == 2:
